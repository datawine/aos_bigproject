        ...
        for (i = 0; i < rx_pkts; i++) {
            ...
            if (tx_buffer[port_id].num == MAX_PKTS_BURST_TX) {
                if (port_id == VLAN_PORT) {
                    tx_pkts = vxlan_tx_pkts(port_id, rx_id, tx_buffer[port_id].buffer, MAX_PKTS_BURST_TX);
                } else {
                    // int k;
                    // for (k = 0; k < MAX_PKTS_BURST_TX; k ++)
                    //     printf("size: %u\n", tx_buffer[port_id].buffer[k]->pkt_len);
                    tx_pkts = rte_eth_tx_burst(port_id, rx_id, tx_buffer[port_id].buffer, MAX_PKTS_BURST_TX);
                }
