...
// SLProcessor is a serverless processor
type SLProcessor struct {
	genericProcessor
	warm_start float64
	cold_start float64
	cold_start_ratio float64
}

func NewSLProcessor(warm_start, cold_start, cold_start_ratio float64) *SLProcessor {
	return &SLProcessor{warm_start: warm_start, cold_start: cold_start, cold_start_ratio: cold_start_ratio}
}

func (p *SLProcessor) Run() {
	for {
		req := p.ReadInQueue()
		if rand.Float64() < p.cold_start_ratio {
			p.ctxCost = p.cold_start
		} else {
			p.ctxCost = p.warm_start
		}
