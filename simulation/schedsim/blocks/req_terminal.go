...
func (k *AllKeeper) TerminateReq(req engine.ReqInterface) {
	...
	t := req.GetRequestTime()
	k.requestsTime = append(k.requestsTime, t)
	//fmt.Printf("%v\n", t)
}
...
func (k *AllKeeper) getTotalRequestTime() float64 {
	tmp := 0.0
	for _, v := range k.requestsTime {
		tmp += v
	}
	return tmp
}
...
func (k *AllKeeper) PrintStats() {
	...
	fmt.Printf("Count\tStolen\tAVG\tSTDDev\t50th\t90th\t95th\t99th\tReqs/time_unit\tTotalRequestTime\n")
	...
	fmt.Printf("%v\t", float64(len(k.items))/engine.GetTime())

	fmt.Printf("%v\n", k.getTotalRequestTime())
}
