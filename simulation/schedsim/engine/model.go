...
type timerEvent struct {
	time     float64
	wakeUpCh chan int
	// number of waiting events when this event was enqueued
	idx      int
}
...
type model struct {
	...
	time            float64
	actorCount      int
	// a heap for timer events
	pq              priorityQueue
	// channel for generator and processor to deliver event
	eventChan       chan interface{}
	// processors wait for requests on queues
	blockedInQueues map[QueueInterface]*list.List
	// generator send out requests on queues
	queues          map[QueueInterface]bool
	// record request processing results
	bookkeeping     []Stats
}
...
/* blocked on specified queues and wait for request events */
...
/* wait for a generated event */
func (m *model) waitActor() {
	newEvent := <-m.eventChan
	//both generator and processor deliver timer event
	if timerE, ok := newEvent.(timerEvent); ok {
		heap.Push(&m.pq, &timerE)
		return
	}
	//only processor deliver block event
